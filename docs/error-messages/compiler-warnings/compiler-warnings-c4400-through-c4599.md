---
description: 'Dowiedz się więcej o: ostrzeżenia kompilatora C4400 do C4599'
...
|[Ostrzeżenie kompilatora (poziom 2) C4412](../../error-messages/compiler-warnings/compiler-warning-level-2-c4412.md)|"*Function*": Sygnatura funkcji zawiera typ "*Type*"; Obiekty C++ nie mogą być przekazywane między czystym kodem a mieszaniem i natywnym.|
...
|[Ostrzeżenie kompilatora (poziom 1) C4420](../../error-messages/compiler-warnings/compiler-warning-level-1-c4420.md)|"*checked_operator*": operator nie jest dostępny, zamiast niego używa *operatora "operator*"; Sprawdzanie w czasie wykonywania może być naruszone|
...
|[Ostrzeżenie kompilatora (error) C4430](../../error-messages/compiler-warnings/compiler-warning-c4430.md)|brak specyfikatora typu — zakładany int. Uwaga: C++ nie obsługuje funkcji default-int|
...
|[Ostrzeżenie kompilatora (poziom 4) C4435](../../error-messages/compiler-warnings/compiler-warning-level-4-c4435.md)|"*derived_class*": układ obiektów w obszarze/VD2 zmieni zmieni się ze względu na wirtualną bazę "*BASE_CLASS*"|
|[Ostrzeżenie kompilatora (poziom 1) C4436](../../error-messages/compiler-warnings/compiler-warning-level-1-c4436.md)|dynamiczne \_ rzutowanie z wirtualnej bazy "*BASE_CLASS*" na "*derived_class*" w konstruktorze lub destruktorze może zakończyć się niepowodzeniem z częściowo skonstruowanym obiektem|
|[Ostrzeżenie kompilatora (poziom 4) C4437](../../error-messages/compiler-warnings/compiler-warning-level-4-c4437.md)|dynamiczne \_ rzutowanie z wirtualnej bazy "*BASE_CLASS*" na "*derived_class*" może zakończyć się niepowodzeniem w niektórych kontekstach|
...
|[Ostrzeżenie kompilatora (error) C4439](../../error-messages/compiler-warnings/compiler-warning-c4439.md)|"*Function*": definicja funkcji z typem zarządzanym w podpisie musi mieć __clrcall konwencją wywoływania|
|[Ostrzeżenie kompilatora (poziom 1) C4440](../../error-messages/compiler-warnings/compiler-warning-level-1-c4440.md)|Zignorowano ponowną definicję konwencji wywoływania z "*calling_convention1*" do "*calling_convenction2*"|
|[Ostrzeżenie kompilatora (poziom 1) C4441](../../error-messages/compiler-warnings/compiler-warning-level-1-c4441.md)|Zignorowano konwencję wywoływania "*calling_convention1*"; Zamiast tego użyto "*calling_convention2*"|
...
|[Ostrzeżenie kompilatora (poziom 1) C4445](../../error-messages/compiler-warnings/compiler-warning-level-1-c4445.md)|"*Function*": w typie "WinRT&#124;Managed" metoda wirtualna nie może być prywatna|
...
|Ostrzeżenie kompilatora (poziom 1) C4447|znaleziono podpis "Main" bez modelu wątkowości. Rozważ użycie "int Main (platform:: Array \<Platform::String^> ^ args)".|
...
|Ostrzeżenie kompilatora (poziom 4) C4451|"classname1:: member": użycie klasy ref "classname2:: member" wewnątrz tego kontekstu może prowadzić do nieprawidłowego kierowania obiektu w kontekstach|
...
|Ostrzeżenie kompilatora (poziom 1) C4453|"*Type*": typ "[WebHostHidden]" nie powinien być używany na publikowanej powierzchni typu publicznego, który nie jest "[WebHostHidden]"|
...
|[Ostrzeżenie kompilatora (poziom 4) C4460](../../error-messages/compiler-warnings/compiler-warning-level-4-c4460.md)|*Operator*"operator" Managed "WinRT&#124;" ma parametr zakończony przez odwołanie. *Operator*"operator" zarządzanej "WinRT&#124;" ma inną semantykę z operatora C++ "*cpp_operator*", czy chodziło o wartość?|
...
|[Ostrzeżenie kompilatora (poziom 4) C4463](compiler-warning-level-4-c4463.md)|przepływ Przypisywanie "*Value*" do pola bitowego, które może zawierać tylko wartości z "*MIN_VALUE*" do "*MAX_VALUE*"|
...
|Ostrzeżenie kompilatora (poziom 1) C4472|element "*Identifier*" jest natywnym wyliczeniem: Dodaj specyfikator dostępu (Private/Public), aby zadeklarować Wyliczenie "WinRT&#124;Managed"|
...
|[Ostrzeżenie kompilatora (poziom 4) C4487](../../error-messages/compiler-warnings/compiler-warning-level-4-c4487.md)|"*derived_class_function*": pasuje do dziedziczonej metody niewirtualnej "*base_class_function*", ale nie jest jawnie oznaczona modyfikatorem "New"|
|[Ostrzeżenie kompilatora (poziom 1) C4488](../../error-messages/compiler-warnings/compiler-warning-level-1-c4488.md)|"*Function*": wymaga słowa kluczowego "*Keyword*", aby zaimplementować metodę interfejsu "*interface_method*"|
...
|Ostrzeżenie kompilatora (poziom 1) C4494|"*Function*": ignorowanie __declspec (Alokator), ponieważ typ zwracany funkcji nie jest wskaźnikiem ani odwołaniem|
...
|[Ostrzeżenie kompilatora (poziom 1) C4530](../../error-messages/compiler-warnings/compiler-warning-level-1-c4530.md)|Użyto programu obsługi wyjątków języka C++, ale nie włączono semantyki operacji unwind. Określ/EHsc|
|Ostrzeżenie kompilatora (poziom 1) C4531|Obsługa wyjątków C++ nie jest dostępna na Windows CE. Użyj strukturalnej obsługi wyjątków|
...
|[Ostrzeżenie kompilatora (poziom 1) C4540](../../error-messages/compiler-warnings/compiler-warning-level-1-c4540.md)|dynamic_cast używany do konwersji na niedostępną lub niejednoznaczną podstawę; test czasu wykonywania zakończy się niepowodzeniem ("*Type1*" na "*Type2*")|
...
|[Ostrzeżenie kompilatora (poziom 3) C4543](../../error-messages/compiler-warnings/compiler-warning-level-3-c4543.md)|Zawstrzykiwany tekst został pominięty przez atrybut "No \_ injected_text"|
...
|[Ostrzeżenie kompilatora (poziom 4) C4571](../../error-messages/compiler-warnings/compiler-warning-level-4-c4571.md)|Informacyjne: semantyka catch (...) zmieniła się od Visual C++ 7,1; wyjątki strukturalne (SEH) nie są już przechwytywane|
...
|Ostrzeżenie kompilatora (poziom 1, błąd) C4585|"*Class*": Klasa WinRT "Public ref class" musi być zapieczętowana lub pochodzić z istniejącej niezapieczętowanej klasy|
|Ostrzeżenie kompilatora (poziom 1, błąd) C4586|"*Type*": Typ publiczny nie może być zadeklarowany w przestrzeni nazw najwyższego poziomu o nazwie "Windows"|
...
|Ostrzeżenie kompilatora (poziom 1) C4591|Przekroczono limit *liczby* wywołań "constexpr" (/constexpr: Głębokość \<NUMBER> )|
...
|Ostrzeżenie kompilatora (poziom 1) C4593|"*Function*": przekroczono limit kroku oceny wywołania "constexpr" elementu "*Limit*"; Użyj/constexpr: kroki, \<NUMBER> Aby zwiększyć limit|
