---
description: 'Dowiedz się więcej na temat: biblioteki DLL rozszerzenia MFC'
...
- Plik wykonywalny klienta musi być aplikacją MFC skompilowaną ze `_AFXDLL` zdefiniowanym.
...
- Biblioteki DLL rozszerzeń MFC powinny być kompilowane ze `_AFXEXT` zdefiniowanym. Te siły `_AFXDLL` są również zdefiniowane i gwarantują, że w plikach nagłówkowych MFC są pobierane odpowiednie deklaracje. Gwarantuje również, że `AFX_EXT_CLASS` jest on zdefiniowany jako `__declspec(dllexport)` podczas KOMPILOWANIA biblioteki DLL, która jest niezbędna, jeśli to makro jest używane do deklarowania klas w bibliotece DLL rozszerzenia MFC.
...
Przed wersją 4,0 MFC, ten typ biblioteki DLL został wywołany jako AFXDLL. AFXDLL odwołuje się do `_AFXDLL` symbolu preprocesora, który jest definiowany podczas kompilowania biblioteki DLL.
...
Ze względu na problemy z dekorowanie i eksportowaniem nazw C++ lista eksportu z biblioteki DLL rozszerzenia MFC może różnić się między wersjami Debug i a DLL dla różnych platform. MFCx0.dll sprzedaży detalicznej zawiera około 2 000 punktów wejścia; MFCx0D.dll debugowania zawiera około 3 000 wyeksportowanych punktów wejścia.
...
Podobnie inne globalne Stany MFC, takie jak bieżący plik wykonywalny do ładowania zasobów z, są również udostępniane między aplikacją kliencką i wszystkimi bibliotekami DLL rozszerzenia MFC, a także MFCx0.dll samej.
...
Dynamiczne tworzenie obiektów MFC z daną nazwą MFC jest podobne. Mechanizm deserializacji obiektu MFC musi mieć wszystkie `CRuntimeClass` zarejestrowane obiekty, aby można je było odtworzyć przez dynamiczne tworzenie obiektów C++ dla wymaganego typu w oparciu o to, co zostało wcześniej zapisane.
...
MFCxx.dll zazwyczaj znajduje się na liście zasobów i klas. MFCxx.dll obejmuje wszystkie standardowe zasoby MFC, w tym ciągi monitów dla wszystkich standardowych identyfikatorów poleceń. Umieszczenie go na końcu listy umożliwia Bibliotekom DLL i aplikacji klienckiej nie posiadanie własnej kopii standardowych zasobów MFC, ale w zamian polega na zasobach udostępnionych w MFCxx.dll.
