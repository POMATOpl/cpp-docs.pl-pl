---
description: 'Dowiedz się więcej o: strony właściwości konsolidatora'
...
Przekierowanie na użytkownika wymusza przekierowanie wszystkich zapisów do HKCR do HKEY \_ bieżącego \_ użytkownika (HKCU). Jeśli przekierowanie na użytkownika jest wyłączone, może to spowodować [błąd PRJ0050 kompilacji projektu](../../error-messages/tool-errors/project-build-error-prj0050.md) , gdy program próbuje wykonać zapis do HKCR.
...
Aby uzyskać informacje na temat uzyskiwania dostępu do strony właściwości **Ogólne** konsolidatora, zobacz [Ustawianie kompilatora C++ i właściwości kompilacji w programie Visual Studio](../working-with-project-properties.md).
...
[/ALLOWBIND](allowbind-prevent-dll-binding.md): No ustawia bit w nagłówku dll, który Bind.exe wskazuje, że nie można powiązać obrazu. Nie ma potrzeby powiązania DLL, jeśli został podpisany cyfrowo (powiązanie unieważnia sygnaturę).
...
Opcja [/NOENTRY](noentry-no-entry-point.md)jest wymagana do utworzenia biblioteki DLL tylko dla zasobów. Użyj tej opcji, aby uniemożliwić łączenie z odwołaniem do `_main` biblioteki DLL.
...
Umożliwia dostarczenie informacji o wewnętrznym błędzie kompilatora (lodem) bezpośrednio do zespołu Visual C++.
