---
description: 'Dowiedz się więcej na temat: używanie pól wywołania zwrotnego w kontrolce selektora dat i godzin'
...
Zainicjuj ciąg niestandardowy wywołaniem metody `CDateTimeCtrl::SetFormat` . Aby uzyskać więcej informacji, zobacz [Używanie niestandardowych ciągów formatu w kontrolce selektora dat i godzin](../mfc/using-custom-format-strings-in-a-date-and-time-picker-control.md). Typowym miejscem, w którym można ustawić ciąg formatu niestandardowego, jest `OnInitDialog` Funkcja klasy dialogowej lub funkcji zawierającej `OnInitialUpdate` klasy widoku.

## <a name="handling-the-dtn_formatquery-notification"></a>Obsługa powiadomienia DTN_FORMATQUERY

Gdy formant analizuje ciąg formatu i napotka pole wywołania zwrotnego, aplikacja wysyła DTN_FORMAT i DTN_FORMATQUERY komunikatów powiadomień. Ciąg pola wywołania zwrotnego jest dołączany do powiadomień, dzięki czemu można określić, które pole wywołania zwrotnego jest wysyłane.
...
Po obliczeniu rozmiaru bieżącego pola wywołania zwrotnego należy podać wartość dla tego pola. Jest to wykonywane w programie obsługi dla powiadomienia DTN_FORMAT.
