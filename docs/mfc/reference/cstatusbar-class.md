---
description: 'Dowiedz się więcej na temat: Klasa CStatusBar'
...
|[CStatusBar:: CreateEx](#createex)|Tworzy `CStatusBar` obiekt z dodatkowymi stylami dla osadzonego `CStatusBarCtrl` obiektu.|
...
1. Wywołaj funkcję [Create](#create) (lub [CreateEx](#createex)), aby utworzyć okno pasek stanu i dołączyć je do `CStatusBar` obiektu.

1. Wywołaj metodę [Setwskaźniks](#setindicators) , aby skojarzyć identyfikator ciągu z każdym wskaźnikiem.
...
1. Wywołanie [CCmdUI:: SetText](../../mfc/reference/ccmdui-class.md#settext) na pasku stanu ON_UPDATE_COMMAND_UI procedury obsługi.

1. Wywołaj [SetPaneText](#setpanetext) w celu zaktualizowania tekstu w dowolnym okienku.

Wywołanie metody [Setokienks](#setpanestyle) w celu zaktualizowania stylu okienka paska stanu.

Aby uzyskać więcej informacji na temat korzystania z programu `CStatusBar` , zobacz artykuł [implementacja paska stanu artykułu w MFC](../../mfc/status-bar-implementation-in-mfc.md) i [Uwaga techniczna 31: Paski kontroli](../../mfc/tn031-control-bars.md).
...
Użyj `CreateEx` zamiast [tworzenia](#create), gdy pewne style muszą być obecne podczas tworzenia osadzonej kontrolki pasek stanu. Na przykład ustaw *dwCtrlStyle* na SBT_TOOLTIPS, aby wyświetlić etykietki narzędzi w obiekcie pasek stanu.
...
`itemAction`Element członkowski `DRAWITEMSTRUCT` struktury definiuje akcję rysowania, która ma zostać wykonana. Przesłoń tę funkcję elementu członkowskiego, aby zaimplementować rysowanie dla obiektu rysowania przez właściciela `CStatusBar` . Aplikacja powinna przywrócić wszystkie obiekty interfejsu GDI (Graphics Device Interface) wybrane dla kontekstu wyświetlania dostarczonego w *lpDrawItemStruct* przed zakończeniem tej funkcji elementu członkowskiego.
...
*lpRect*<br/>
Wskazuje strukturę [Rect](/windows/win32/api/windef/ns-windef-rect) lub obiekt [CRect](../../atl-mfc-shared/reference/crect-class.md) , który będzie otrzymywać współrzędne wskaźnika określonego przez *nIndex*.
...
## <a name="cstatusbargetpanestyle"></a><a name="getpanestyle"></a> CStatusBar:: getokienks
...
- SBPS_NOBORDERS żadne obramowanie trójwymiarowe wokół okienka.

- SBPS_POPOUT odwrócić obramowanie, aby tekst "pop out".

- SBPS_DISABLED nie Rysuj tekstu.

- SBPS_STRETCH rozciągane okienko, aby wypełnić nieużywane miejsce. Tylko jedno okienko na pasek stanu może mieć ten styl.

- Nie SBPS_NORMAL rozciągania, obramowań ani wyskakujących okienek.

## <a name="cstatusbarsetpanestyle"></a><a name="setpanestyle"></a> CStatusBar:: setokienks
...
Po wywołaniu należy `SetPaneText` dodać program obsługi aktualizacji interfejsu użytkownika w celu wyświetlenia nowego tekstu na pasku stanu.
