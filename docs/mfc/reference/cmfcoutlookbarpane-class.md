---
description: 'Dowiedz się więcej na temat: Klasa CMFCOutlookBarPane'
...
Aby skonstruować `CMFCOutlookBarPane` obiekt, najpierw Wywołaj konstruktora, a następnie Wywołaj `Create` , który tworzy formant okienka paska programu Outlook i dołącza go do `CMFCOutlookBarPane` obiektu.
...
Ta metoda przywraca oryginalny zestaw przycisków paska programu Outlook. Ta metoda jest taka sama jak `CMFCOutlookBarPane::RestoreOriginalstate` , z tą różnicą, że nie wyzwala ponownego rysowania okienka paska programu Outlook.
