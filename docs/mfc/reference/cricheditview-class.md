---
description: 'Dowiedz się więcej na temat: Klasa CRichEditView'
...
## <a name="cricheditviewcanpaste"></a><a name="canpaste"></a> CRichEditView:: Ostatnia próba
...
## <a name="cricheditviewfindtext"></a><a name="findtext"></a> CRichEditView:: ciąg FindText
...
Jeśli wartość zwracana wskazuje powodzenie, `IRichEditOleCallback::GetClipboardData` zwraca `IDataObject` dostępną przez *lplpdataobj*; w przeciwnym razie zwraca wartość dostępną przez *lpRichDataObj*. Zastąp tę funkcję, aby podać własne dane Schowka. Domyślna implementacja tej funkcji zwraca E_NOTIMPL.
...
## <a name="cricheditviewgetcontextmenu"></a><a name="getcontextmenu"></a> CRichEditView::
...
Wywołaj tę funkcję, aby pobrać element OLE, który jest aktualnie aktywowany w miejscu w tym `CRichEditView` obiekcie.
...
Wywołaj tę funkcję, aby pobrać element OLE ( `CRichEditCntrItem` obiekt), który jest aktualnie wybrany w tym `CRichEditView` obiekcie.
...
Wywołaj tę funkcję elementu członkowskiego, aby obliczyć długość tekstu w tym `CRichEditView` obiekcie.
...
Wywołaj tę funkcję, aby ustawić atrybuty formatowania znaku dla nowego tekstu w tym `CRichEditView` obiekcie.
