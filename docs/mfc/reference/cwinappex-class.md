---
description: 'Dowiedz się więcej na temat: Klasa CWinAppEx'
...
|[CWinAppEx:: ExitInstance](#exitinstance)|Wywoływane przez platformę z poziomu `Run` funkcji składowej, aby wyjść z tego wystąpienia aplikacji. (Przesłania [CWinApp:: ExitInstance](../../mfc/reference/cwinapp-class.md#exitinstance).)|
...
|[CWinAppEx:: GetObject](#getobject)|Odczytuje `CObject` dane pochodne skojarzone z określoną wartością z rejestru.|
...
|[CWinAppEx::WriteSectionObject](#writesectionobject)|Zapisuje dane pochodne od `CObject` klasy do wartości określonego klucza rejestru.|
...
Większość funkcji zapewnianych przez strukturę MFC zależy od `CWinAppEx` klasy. Możesz dołączyć `CWinAppEx` klasę do aplikacji na jeden z dwóch sposobów:

- Konstruowanie `CWinAppEx` klasy w wątku głównym.

- Utwórz główną klasę aplikacji z `CWinAppEx` .

Po dołączeniu `CWinAppEx` do aplikacji można zainicjować jednego z menedżerów aplikacji. Przed użyciem Menedżera aplikacji należy go zainicjować, wywołując odpowiednią metodę Initialize. Aby uzyskać wskaźnik do określonego Menedżera, wywołaj skojarzoną metodę get. `CWinAppEx`Klasa zarządza następującymi menedżerami aplikacji: [Klasa CMouseManager](../../mfc/reference/cmousemanager-class.md), [Klasa CContextMenuManager](../../mfc/reference/ccontextmenumanager-class.md), Klasa [CKeyboardManager](../../mfc/reference/ckeyboardmanager-class.md), [Klasa CUserToolsManager](../../mfc/reference/cusertoolsmanager-class.md)i [Klasa CMenuTearOffManager](../../mfc/reference/cmenutearoffmanager-class.md).
...
Ta metoda czyści dane aplikacji z określonej sekcji rejestru. Można określić sekcję do wyczyszczenia przy użyciu parametru *lpszSectionName*. Jeśli *lpszSectionName* ma wartość null, ta metoda będzie używać domyślnej ścieżki rejestru przechowywanej w `CWinAppEx` obiekcie. Aby uzyskać domyślną ścieżkę rejestru, użyj [CWinAppEx:: GetRegistryBase](#getregistrybase).
...
`CWinAppEx`Klasa ma metody inicjacji, zapewnia funkcjonalność zapisywania i ładowania informacji o aplikacji do rejestru oraz kontroluje globalne ustawienia aplikacji. Umożliwia również korzystanie z menedżerów globalnych, takich jak [Klasa CKeyboardManager](../../mfc/reference/ckeyboardmanager-class.md) i [Klasa CUserToolsManager](../../mfc/reference/cusertoolsmanager-class.md). Każda aplikacja może mieć tylko jedno wystąpienie `CWinAppEx` klasy.
...
*pToolRTC*<br/>
podczas Klasa, za `CUserToolsManager` pomocą której obiekt tworzy nowe narzędzia użytkownika.
...
TRUE, jeśli metoda tworzy i inicjuje `CUserToolsManager` obiekt; FAŁSZ, jeśli metoda nie powiedzie się lub jeśli `CUserToolsManager` obiekt już istnieje.
...
## <a name="cwinappexgetcontextmenumanager"></a><a name="getcontextmenumanager"></a> CWinAppEx::
...
Wskaźnik do `CContextMenuManager` obiektu globalnego.
...
## <a name="cwinappexgetint"></a><a name="getint"></a> CWinAppEx:: metod GetInt
...
Wskaźnik do `CKeyboardManager` obiektu globalnego.
...
Wskaźnik do `CMouseManager` obiektu globalnego.
...
Ta metoda odczytuje dane z rejestru, który pochodzi od `CObject` . Aby zapisać `CObject` dane w rejestrze, użyj elementu [CWinAppEx:: WriteObject](#writeobject) lub [CWinAppEx:: WriteSectionObject](#writesectionobject).
...
*obiektów*<br/>
określoną Odwołanie do `CObject` . Ta metoda służy `CObject` do przechowywania danych rejestru.
...
Ta metoda odczytuje dane z rejestru. Odczytane dane to `CObject` dane lub dane dla klasy pochodzącej od `CObject` . Aby zapisać `CObject` dane w rejestrze, użyj elementu [CWinAppEx:: WriteObject](#writeobject) lub [CWinAppEx:: WriteSectionObject](#writesectionobject).
...
Wskaźnik do `CShellManager` obiektu globalnego.

### <a name="remarks"></a>Uwagi

Jeśli `CShellManager` obiekt nie zostanie zainicjowany, ta funkcja wywołuje [CWinAppEx:: InitShellManager](#initshellmanager) przed zwróceniem wskaźnika.
...
Wskaźnik do `CTooltipManager` obiektu globalnego.

### <a name="remarks"></a>Uwagi

Jeśli `CTooltipManager` obiekt nie zostanie zainicjowany, ta funkcja wywołuje [CWinAppEx:: InitTooltipManager](#inittooltipmanager) przed zwróceniem wskaźnika.
...
Wskaźnik do `CUserToolsManager` obiektu globalnego; Wartość NULL, jeśli zarządzanie narzędziami użytkownika nie jest włączone dla aplikacji.

### <a name="remarks"></a>Uwagi

Przed pobraniem wskaźnika do `CUserToolsManager` obiektu należy zainicjować Menedżera przez wywołanie [CWinAppEx:: EnableUserTools](#enableusertools).
...
Niezerowe, jeśli metoda tworzy obiekt CContextMenuManager; 0, jeśli `CContextMenuManager` obiekt już istnieje.

### <a name="remarks"></a>Uwagi

Jeśli wywołasz metodę [CWinAppEx::](#getcontextmenumanager), domyślna implementacja tej metody wywołuje `InitContextMenuManager` .

Jeśli aplikacja ma już Menedżera menu kontekstowego i jest wywoływana `InitContextMenuManager` , aplikacja będzie miała błąd [potwierdzenia](diagnostic-services.md#assert) . W związku z tym nie należy wywoływać `InitContextMenuManager` w przypadku `CContextMenuManager` bezpośredniego tworzenia obiektu. Jeśli nie używasz niestandardowego `CContextMenuManager` , użyj polecenia, `GetContextMenuManager` Aby utworzyć `CContextMenuManager` obiekt.
...
Różne od zera, jeśli metoda tworzy `CKeyboardManager` obiekt; 0 Jeśli `CKeyboardManager` obiekt już istnieje.
...
Jeśli aplikacja ma już Menedżera klawiatury i jest wywoływana `InitKeyboardManager` , aplikacja będzie miała błąd [potwierdzenia](diagnostic-services.md#assert) . W związku z tym nie należy wywoływać `InitKeyboardManager` w przypadku `CKeyboardManager` bezpośredniego tworzenia obiektu. Jeśli nie używasz niestandardowego `CKeyboardManager` , użyj polecenia, `GetKeyboardManager` Aby utworzyć `CKeyboardManager` obiekt.
...
Różne od zera, jeśli metoda tworzy `CMouseManager` obiekt; 0 Jeśli `CMouseManager` obiekt już istnieje.

### <a name="remarks"></a>Uwagi

Jeśli wywołasz [CWinAppEx:: GetMouseManager](#getmousemanager), domyślna implementacja tej metody wywołuje `InitMouseManager` .

Jeśli aplikacja ma już Menedżera myszy i jest wywoływana `InitMouseManager` , aplikacja będzie miała błąd [potwierdzenia](diagnostic-services.md#assert) . W związku z tym nie należy wywoływać `InitMouseManager` w przypadku `CMouseManager` bezpośredniego tworzenia obiektu. Jeśli nie używasz niestandardowego `CMouseManager` , użyj polecenia, `GetMouseManager` Aby utworzyć `CMouseManager` obiekt.
...
Różne od zera, jeśli metoda tworzy `CShellManager` obiekt; 0 Jeśli `CShellManager` obiekt już istnieje.

### <a name="remarks"></a>Uwagi

Jeśli wywołasz [CWinAppEx:: GetShellManager](#getshellmanager), domyślna implementacja tej metody wywołuje `InitShellManager` .

Jeśli aplikacja ma już Menedżera powłoki i jest wywoływana `InitShellManager` , aplikacja zgłasza błąd [potwierdzenia](diagnostic-services.md#assert) . W związku z tym nie należy wywoływać `InitShellManager` w przypadku `CShellManager` bezpośredniego tworzenia obiektu. Jeśli nie używasz niestandardowego `CShellManager` , użyj polecenia, `GetShellManager` Aby utworzyć `CShellManager` obiekt.
...
Różne od zera, jeśli metoda tworzy `CTooltipManager` obiekt; 0 Jeśli `CTooltipManager` obiekt już istnieje.
...
Jeśli aplikacja ma już Menedżera etykietki narzędzi i jest wywoływana `InitTooltipManager` , aplikacja będzie miała błąd [potwierdzenia](diagnostic-services.md#assert) . W związku z tym nie należy wywoływać `InitTooltipManager` w przypadku `CTooltipManager` bezpośredniego tworzenia obiektu. Jeśli nie używasz niestandardowego `CTooltipManager` , użyj polecenia, `GetTooltipManager` Aby utworzyć `CTooltipManager` obiekt.
...
*pFrameImpl*<br/>
podczas Wskaźnik do `CFrameImpl` obiektu. Metoda stosuje informacje o stanie w rejestrze do tego okna ramki.
...
Domyślna implementacja `CFrameImpl::OnLoadFrame` wywołań `LoadState` .
...
`m_bForceImageReset`Element członkowski danych jest zmienną chronioną.
...
*pFrameImpl*<br/>
podczas Wskaźnik do `CFrameImpl` obiektu.
...
*pFrameImpl*<br/>
podczas Wskaźnik do `CFrameImpl` obiektu. Ta ramka jest zapisywana w rejestrze systemu Windows.
...
Aby obsłużyć menu podręczne, musisz mieć obiekt [CContextMenuManager](../../mfc/reference/ccontextmenumanager-class.md) . Jeśli obiekt nie został zainicjowany `CContextMenuManager` , zakończy `ShowPopupMenu` się niepowodzeniem.
