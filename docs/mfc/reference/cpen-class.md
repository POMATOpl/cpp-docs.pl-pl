---
description: 'Dowiedz się więcej na temat: Klasa CPen'
title: Klasa CPen
ms.date: 11/04/2016
f1_keywords:
- CPen
...
|[CPen::FromHandle](#fromhandle)|Zwraca wskaźnik do obiektu, w `CPen` którym znajduje się HPEN systemu Windows.|
...
Aby uzyskać więcej informacji o używaniu programu `CPen` , zobacz [grafika Objects](../../mfc/graphic-objects.md).
...
- PS_ENDCAP_ROUND zakończenia są zaokrąglane.
...
- PS_JOIN_MITER joins są mitered, gdy znajdują się w bieżącym limicie ustawionym przez funkcję [SetMiterLimit](/windows/win32/api/wingdi/nf-wingdi-setmiterlimit) . Jeśli sprzężenie przekracza ten limit, zostanie on skośny.
...
*pLogBrush*<br/>
Wskazuje `LOGBRUSH` strukturę. Jeśli *nPenStyle* jest PS_COSMETIC, element członkowski *lbColor* `LOGBRUSH` struktury określa kolor pióra, a element członkowski *lbStyle* `LOGBRUSH` struktury musi być ustawiony na BS_SOLID. Jeśli *nPenStyle* jest PS_GEOMETRIC, wszystkie elementy członkowskie muszą być używane do określania atrybutów pędzla pióra.
...
Jeśli używasz konstruktora bez argumentów, musisz zainicjować otrzymany `CPen` obiekt za pomocą `CreatePen` `CreatePenIndirect` funkcji, lub `CreateStockObject` .
...
*pLogBrush*<br/>
Wskazuje na strukturę [LOGBRUSH](/windows/win32/api/wingdi/ns-wingdi-logbrush) . Jeśli *nPenStyle* jest PS_COSMETIC, `lbColor` element członkowski `LOGBRUSH` struktury określa kolor pióra, a element członkowski *lbStyle* `LOGBRUSH` struktury musi być ustawiony na BS_SOLID. Jeśli nPenStyle jest PS_GEOMETRIC, wszystkie elementy członkowskie muszą być używane do określania atrybutów pędzla pióra.
...
Jeśli pióro ma styl PS_INSIDEFRAME i kolor, który nie pasuje do koloru w logicznej tabeli kolorów, pióro jest rysowane kolorem. Styl pióra PS_SOLID nie może służyć do tworzenia pióra z kolorem. Styl PS_INSIDEFRAME jest identyczny z PS_SOLID, jeśli szerokość pióra jest mniejsza lub równa 1.
...
Zwraca wskaźnik do obiektu, który `CPen` ma uchwyt do obiektu pióra GDI systemu Windows.
...
Pobiera `EXTLOGPEN` podstawową strukturę.
...
Pobiera `LOGPEN` podstawową strukturę.
...
Pobiera dołączone dojście GDI systemu Windows `CPen` obiektu.
...
Jeśli to się powiedzie, dojście do obiektu GDI systemu Windows reprezentowane przez `CPen` obiekt; w przeciwnym razie wartość null.
