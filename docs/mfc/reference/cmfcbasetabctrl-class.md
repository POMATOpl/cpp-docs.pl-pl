---
description: 'Dowiedz się więcej na temat: Klasa CMFCBaseTabCtrl'
...
## <a name="cmfcbasetabctrlcreatewrapper"></a><a name="createwrapper"></a> CMFCBaseTabCtrl:: isotoka
...
Okno z kartami może zadokować dowolny obiekt pochodzący z `CWnd` . Aby jednak `CMFCBaseTabCtrl Class` obiekt był było dokować, każdy obiekt w elemencie `CMFCBaseTabCtrl` musi być odłączony. W związku `CMFCBaseTabCtrl` z tym program automatycznie otacza wszystkie obiekty, które nie są wyprowadzane z `CDockablePane` .
...
Domyślnie ta metoda nie jest obsługiwana przez program `CMFCBaseTabCtrl Class` . Ta funkcja powinna zostać zaimplementowana w klasie niestandardowej pochodnej od `CMFCBaseTabCtrl` , jeśli ta kontrolka karty niestandardowej obsługuje przewijanie karty. Ta metoda jest obsługiwana przez [klasę CMFCTabCtrl](../../mfc/reference/cmfctabctrl-class.md).
...
W `CMFCBaseTabCtrl` klasie ta metoda jest czystą funkcją wirtualną: należy ją zaimplementować, jeśli pochodna jest Klasa `CMFCBaseTabCtrl` .
...
Po `GetTabArea` powrocie `CRect` parametry zawierają rozmiar i położenie obszaru karty w współrzędnej klienta kontrolki karta. Jeśli nie ma obszaru karty w górnej lub dolnej części kontrolki karta `rectTabAreaTop` lub `rectTabAreaBottom` jest pusta.
...
Ta metoda pobiera bezpośredni wskaźnik do `CWnd` obiektu, który został dodany przy użyciu metody [CMFCBaseTabCtrl:: AddTab](#addtab) lub [CMFCBaseTabCtrl:: InsertTab](#inserttab). `GetTabWndNoWrapper` Pobiera wskaźnik do dodanego elementu `CWnd` , nawet jeśli struktura dodała otokę dla obiektu. Aby uzyskać więcej informacji na temat otok i [klasy CMFCBaseTabCtrl](../../mfc/reference/cmfcbasetabctrl-class.md), zobacz [CMFCBaseTabCtrl:: setotoka](#createwrapper).
...
## <a name="cmfcbasetabctrliscolored"></a><a name="iscolored"></a> CMFCBaseTabCtrl:: iscolord
...
## <a name="cmfcbasetabctrlisonenotestyle"></a><a name="isonenotestyle"></a> CMFCBaseTabCtrl:: isonenote
...
## <a name="cmfcbasetabctrlserialize"></a><a name="serialize"></a> CMFCBaseTabCtrl:: serializować
...
Parametr *bActivate* ma zastosowanie tylko wtedy, gdy *bShow* ma wartość true. Jeśli *bActivate* ma wartość true i jeśli zakończy `ShowTab` się pomyślnie, wyśle `ShowTab` komunikat AFX_WM_CHANGE_ACTIVE_TAB do elementu nadrzędnego okna karty.
