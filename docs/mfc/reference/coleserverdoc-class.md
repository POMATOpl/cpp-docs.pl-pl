---
description: 'Dowiedz się więcej na temat: Klasa COleServerDoc'
...
|[COleServerDoc::GetDocObjectServer](#getdocobjectserver)|Przesłoń tę funkcję, aby utworzyć nowy `CDocObjectServer` obiekt i wskazać, że ten dokument jest kontenerem DocObject.|
...
|[COleServerDoc:: OnGetEmbeddedItem](#ongetembeddeditem)|Wywołuje się, by uzyskać `COleServerItem` element reprezentujący cały dokument; służy do uzyskiwania elementu osadzonego. Wymagana implementacja.|
...
Dokument serwera może zawierać obiekty [COleServerItem](../../mfc/reference/coleserveritem-class.md) reprezentujące interfejs serwera do elementów osadzonych lub połączonych. Gdy aplikacja serwera jest uruchamiana przez kontener w celu edycji elementu osadzonego, element jest ładowany jako własny dokument serwera; `COleServerDoc` obiekt zawiera tylko jeden `COleServerItem` obiekt, składający się z całego dokumentu. Gdy aplikacja serwera jest uruchamiana przez kontener w celu edycji elementu połączonego, istniejący dokument jest ładowany z dysku; część zawartości dokumentu zostanie wyróżniona w celu wskazania połączonego elementu.

`COleServerDoc` obiekty mogą również zawierać elementy klasy [COleClientItem](../../mfc/reference/coleclientitem-class.md) . Dzięki temu można tworzyć aplikacje kontenerów serwerów. Struktura zawiera funkcje do prawidłowego przechowywania `COleClientItem` elementów podczas obsługi `COleServerItem` obiektów.
...
Aby można było obsługiwać połączone elementy, `COleServerDoc` udostępnia funkcję członkowską [OnGetLinkedItem](../../mfc/reference/colelinkingdoc-class.md#ongetlinkeditem) . Możesz użyć domyślnej implementacji lub zastąpić ją, jeśli masz własny sposób zarządzania elementami dokumentu.

Wymagana jest `COleServerDoc` jednopochodna Klasa dla każdego typu dokumentu serwera obsługiwanego przez aplikację. Na przykład jeśli aplikacja serwera obsługuje arkusze kalkulacyjne i wykresy, potrzebne są `COleServerDoc` klasy pochodne.
...
Domyślnie program nie `COleServerDoc` obsługuje dokumentów aktywnych (nazywanych również DocObjects). Aby włączyć tę pomoc techniczną, zobacz [GetDocObjectServer](#getdocobjectserver) i Class [CDocObjectServer](../../mfc/reference/cdocobjectserver-class.md).
...
Przesłoń tę funkcję, aby utworzyć nowy `CDocObjectServer` element i zwrócić do niego wskaźnik.
...
Typowa implementacja dokumentu, który obsługuje DocObjects, spowoduje po prostu przydzielenie nowego `CDocObjectServer` obiektu i przywrócenie go do obiektu wywołującego. Na przykład:
...
*lpClipRect*<br/>
Wskaźnik do `RECT` struktury lub obiektu, `CRect` Aby uzyskać współrzędne prostokąta wycinka elementu.
...
*lpPosRect*<br/>
Wskaźnik do `RECT` struktury lub obiektu, `CRect` Aby otrzymać współrzędne elementu.
...
`GetZoomFactor`Funkcja członkowska określa "współczynnik powiększenia" elementu, który został aktywowany do edycji w miejscu.
...
*lpSizeNum*<br/>
Wskaźnik do obiektu klasy, w `CSize` którym będzie przechowywany licznik współczynnika powiększenia. Może mieć wartość NULL.
...
Funkcja ustawia swoje pierwsze dwa argumenty do licznika i mianownika "współczynnika powiększenia" elementu. Jeśli element nie jest edytowany w miejscu, funkcja ustawia te argumenty na wartość domyślną 100% (lub 1:1) i zwraca zero. Aby uzyskać więcej informacji, zobacz Uwaga techniczna 40, [MFC/OLE In-Place zmiana rozmiaru i powiększanie](../../mfc/tn040-mfc-ole-in-place-resizing-and-zooming.md).
...
Różne od zera, jeśli `COleServerDoc` obiekt jest dokumentem, który reprezentuje obiekt osadzony w kontenerze; w przeciwnym razie 0.
...
`COleCmdUI` może służyć do włączania, aktualizowania i ustawiania innych właściwości poleceń interfejsu użytkownika DocObject. Po zainicjowaniu poleceń można wykonać je za pomocą polecenia `OnExecOleCmd` .
...
Zastąp tę funkcję, jeśli aplikacja obsługuje cofanie. Zazwyczaj można wykonać operację cofania, a następnie aktywować element, wywołując metodę `ActivateInPlace` . Jeśli aplikacja kontenera jest zapisywana przy użyciu biblioteka MFC, wywołanie `COleClientItem::ReactivateAndUndo` powoduje wywołanie tej funkcji.
...
*lpPosRect*<br/>
Wskaźnik do `RECT` struktury lub `CRect` obiektu, który określa położenie okna ramki w miejscu względem obszaru klienckiego aplikacji kontenera.

*lpClipRect*<br/>
Wskaźnik do `RECT` struktury lub `CRect` obiektu, który określa prostokąt przycinania okna ramki w miejscu względem obszaru klienckiego aplikacji kontenera.
...
Ta funkcja jest wywoływana automatycznie z programu `OnUpdateDocument` . Należy zauważyć, że ta funkcja powoduje aktualizację elementu na dysku, więc jest zazwyczaj wywoływana tylko w wyniku określonej akcji użytkownika.

## <a name="coleserverdocscrollcontainerby"></a><a name="scrollcontainerby"></a> COleServerDoc::ScrollContainerBy

Wywołaj `ScrollContainerBy` funkcję elementu członkowskiego, aby przewinąć dokument kontenera o ilość w pikselach wskazywanych przez `sizeScroll` .
...
Ta funkcja wywołuje `OnUpdate` funkcję członkowską dla każdego z elementów dokumentu z wyjątkiem elementu wysyłającego, przekazując *pHint*, *lHint* i *nDrawAspect*. Te parametry służą do przekazywania informacji o zmianach wprowadzonych do dokumentu. Można kodować informacje przy użyciu *lHint* lub zdefiniować `CObject` klasę pochodną do przechowywania informacji o zmianach i przekazać obiekt tej klasy przy użyciu *pHint*. Przesłoń `OnUpdate` funkcję członkowską w `COleServerItem` klasie pochodnej, aby zoptymalizować aktualizację poszczególnych elementów w zależności od tego, czy prezentacja została zmieniona.
