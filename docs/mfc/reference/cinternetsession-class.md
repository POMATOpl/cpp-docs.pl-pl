---
description: 'Dowiedz się więcej na temat: Klasa CInternetSession'
...
|[CInternetSession:: setcookas](#setcookie)|Ustawia plik cookie dla podanego adresu URL.|
...
W przypadku otwarcia adresu URL na serwerze internetowym przy użyciu programu `OpenURL` można odczytać informacje z witryny. Jeśli chcesz wykonać działania specyficzne dla usługi (na przykład HTTP, FTP lub gopher) dla plików znajdujących się na serwerze, należy ustanowić odpowiednie połączenie z tym serwerem. Aby otworzyć określony rodzaj połączenia bezpośrednio z określoną usługą, należy użyć jednej z następujących funkcji Członkowskich:
...
Metoda [SetOption](#setoption) umożliwia ustawienie opcji zapytania dla sesji, takich jak wartości limitu czasu, liczba ponownych prób i tak dalej.

`CInternetSession` funkcje składowe [setcooka](#setcookie), [getcookas](#getcookie)i [GetCookieLength](#getcookielength) zapewniają środki do zarządzania bazą danych plików cookie Win32, za pośrednictwem których serwery i skrypty utrzymują informacje o stanie na stacji roboczej klienta.

Aby uzyskać więcej informacji o podstawowych zadaniach związanych z programowaniem internetowym, zapoznaj się z artykułem [internetowym pierwsze kroki: WinInet](../../mfc/wininet-basics.md). Aby uzyskać ogólne informacje na temat używania klas MFC WinInet, zapoznaj się z artykułem [programowanie internetowe za pomocą usługi WinInet](../../mfc/win32-internet-extensions-wininet.md).
...
- INTERNET_OPEN_TYPE_PRECONFIG nawiązać połączenia przy użyciu wstępnie skonfigurowanych ustawień w rejestrze. Ten typ dostępu jest ustawiany jako domyślny. Aby nawiązać połączenie za pomocą serwera proxy TIS, ustaw *dwAccessType* na tę wartość. Następnie należy odpowiednio ustawić rejestr.

- INTERNET_OPEN_TYPE_DIRECT połączyć się bezpośrednio z Internetem.

- INTERNET_OPEN_TYPE_PROXY połączyć się za pomocą serwera proxy CERN.
...
*flagiDW*<br/>
Wskazuje różne opcje buforowania. Wartość domyślna to 0. Dopuszczalne wartości to:
...
Aby uzyskać więcej informacji o operacjach asynchronicznych, zobacz artykuł [Internet pierwsze kroki: WinInet](../../mfc/wininet-basics.md).

## <a name="cinternetsessiongetcookie"></a><a name="getcookie"></a> CInternetSession:: getcookas
...
- ERROR_INSUFFICIENT_BUFFER wartość przeniesiona w *dwBufLen* jest niewystarczająca, aby skopiować wszystkie dane plików cookie. Wartość zwrócona w *dwBufLen* to rozmiar buforu niezbędnego do pobrania wszystkich danych.
...
Ta wartość jest używana przez [Getcookas](#getcookie).

## <a name="cinternetsessiongetftpconnection"></a><a name="getftpconnection"></a> CInternetSession::GetFtpConnection

Wywołaj tę funkcję elementu członkowskiego, aby nawiązać połączenie FTP i uzyskać wskaźnik do `CFtpConnection` obiektu.
...
|      NULL       | Ciąg o wartości innej niż NULL |            ERROR            |            ERROR            |
...
*bPassive*<br/>
Określa tryb pasywny lub aktywny dla tej sesji FTP. Jeśli ma wartość TRUE, ustawia Win32 API `dwFlag` na INTERNET_FLAG_PASSIVE.
...
Wywołaj tę funkcję elementu członkowskiego, aby nawiązać nowe połączenie Gopher i uzyskać wskaźnik do `CGopherConnection` obiektu.
...
Wywołaj tę funkcję elementu członkowskiego, aby nawiązać połączenie HTTP i uzyskać wskaźnik do `CHttpConnection` obiektu.
...
- INTERNET_FLAG_RELOAD pobierać dane z przewodu, nawet jeśli są buforowane lokalnie.
...
- INTERNET_OPEN_FLAG_USE_EXISTING_CONNECT Jeśli to możliwe, Użyj istniejących połączeń z serwerem w celu uzyskania nowych żądań wygenerowanych przez program `OpenUrl` zamiast tworzenia nowej sesji dla każdego żądania połączenia.
...
Wskaźnik, który `OpenURL` zwraca, zależy od typu usługi *pstrURL*. W poniższej tabeli pokazano, jakie możliwe wskaźniki `OpenURL` można zwrócić.
...
Parametr *flagiDW* musi zawierać INTERNET_FLAG_TRANSFER_ASCII lub INTERNET_FLAG_TRANSFER_BINARY, ale nie oba. Pozostałe flagi można łączyć **z operatorem bitowym or (** **&#124;**).

`OpenURL`, który zawija funkcję Win32 `InternetOpenURL` , umożliwia pobieranie, pobieranie i odczytywanie danych z serwera internetowego. `OpenURL` nie zezwala na manipulowanie plikami w lokalizacji zdalnej, dlatego nie wymaga żadnych obiektów [CInternetConnection](../../mfc/reference/cinternetconnection-class.md) .
...
## <a name="cinternetsessionsetcookie"></a><a name="setcookie"></a> CInternetSession:: setcookas
