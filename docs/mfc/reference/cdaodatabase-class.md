---
description: 'Dowiedz się więcej na temat: Klasa CDaoDatabase'
...
|[CDaoDatabase:: Update](#canupdate)|Zwraca wartość różną od zera, jeśli `CDaoDatabase` obiekt jest aktualizowalny (nie tylko do odczytu).|
...
|[CDaoDatabase:: IsOpen](#isopen)|Zwraca wartość różną od zera, jeśli `CDaoDatabase` obiekt jest obecnie połączony z bazą danych.|
...
Obiekty bazy danych można tworzyć niejawnie, podczas tworzenia obiektów zestawu rekordów. Możesz również jawnie tworzyć obiekty bazy danych. Aby jawnie użyć istniejącej bazy danych `CDaoDatabase` , wykonaj jedną z następujących czynności:

- Konstruowanie `CDaoDatabase` obiektu, przekazanie wskaźnika do otwartego obiektu [CDaoWorkspace](../../mfc/reference/cdaoworkspace-class.md) .
...
## <a name="cdaodatabasecantransact"></a><a name="cantransact"></a> CDaoDatabase:: gettransact
...
## <a name="cdaodatabasecanupdate"></a><a name="canupdate"></a> CDaoDatabase:: Update

Wywołaj tę funkcję elementu członkowskiego, aby określić, czy `CDaoDatabase` obiekt zezwala na aktualizacje.
...
Niezerowe, jeśli `CDaoDatabase` obiekt zezwala na aktualizacje; w przeciwnym razie wartość 0 wskazuje, że wartość true w *bReadOnly* została przeprowadzona podczas otwierania `CDaoDatabase` obiektu lub że sama baza danych jest tylko do odczytu. Zobacz [Otwórz](#open) funkcję członkowską.
...
*pWorkspace*<br/>
Wskaźnik do `CDaoWorkspace` obiektu, który będzie zawierać nowy obiekt bazy danych. W przypadku zaakceptowania domyślnej wartości NULL Konstruktor tworzy `CDaoWorkspace` obiekt tymczasowy, który używa domyślnego obszaru roboczego DAO. Możesz uzyskać wskaźnik do obiektu obszaru roboczego za pośrednictwem elementu członkowskiego danych [m_pWorkspace](#m_pworkspace) .
...
> [!NOTE]
> `CDaoDatabase`Obiekt jest również tworzony niejawnie, jeśli otworzysz obiekt [CDaoRecordset](../../mfc/reference/cdaorecordset-class.md) bez przekazywania wskaźnika do istniejącego `CDaoDatabase` obiektu. Ten obiekt bazy danych jest zamykany po zamknięciu obiektu zestawu rekordów.
...
Dobrym sposobem jest zamknięcie tych obiektów samodzielnie przed wywołaniem tej funkcji elementu członkowskiego. Zamknięcie `CDaoDatabase` obiektu spowoduje usunięcie go z kolekcji baz danych w skojarzonym [obszarze roboczym](../../mfc/reference/cdaoworkspace-class.md). Ponieważ nie `Close` niszczy tego `CDaoDatabase` obiektu, można ponownie użyć tego obiektu, otwierając tę samą bazę danych lub inną bazę danych.

> [!CAUTION]
> Wywołaj funkcję elementu członkowskiego [aktualizacji](../../mfc/reference/cdaorecordset-class.md#update) (jeśli istnieją oczekujące zmiany) i `Close` funkcję członkowską dla wszystkich otwartych obiektów zestawu rekordów przed zamknięciem bazy danych. Jeśli zakończysz funkcję, która deklaruje [CDaoRecordset](../../mfc/reference/cdaorecordset-class.md) lub `CDaoDatabase` obiekty na stosie, baza danych zostanie ZAMKNIĘTA, wszystkie niezapisane zmiany zostaną utracone, wszystkie oczekujące transakcje zostaną wycofane i wszystkie oczekujące zmiany danych zostaną utracone.

> [!CAUTION]
> Jeśli spróbujesz zamknąć obiekt bazy danych w czasie, gdy wszystkie obiekty zestawu rekordów są otwarte lub jeśli spróbujesz zamknąć obiekt obszaru roboczego, gdy wszystkie obiekty bazy danych należące do tego określonego obszaru roboczego są otwarte, te obiekty zestawu rekordów zostaną zamknięte i wszystkie oczekujące aktualizacje lub zmiany zostaną wycofane. Jeśli spróbujesz zamknąć obiekt obszaru roboczego, gdy wszystkie obiekty bazy danych należące do niego są otwarte, operacja zamknie wszystkie obiekty bazy danych należące do tego określonego obiektu obszaru roboczego, co może spowodować zamknięcie niezamkniętych obiektów zestawu rekordów. Jeśli obiekt bazy danych nie zostanie zamknięty, MFC zgłosi błąd potwierdzenia w kompilacjach debugowania.
...
*dwOptions*<br/>
Liczba całkowita, która wskazuje co najmniej jedną opcję. Możliwe wartości:
...
`Create` tworzy plik bazy danych i źródłowy obiekt bazy danych DAO i inicjuje obiekt C++. Obiekt jest dołączany do kolekcji baz danych skojarzonego obszaru roboczego. Obiekt bazy danych jest w stanie otwartym; Nie wywołuj `Open*` po `Create` .

> [!NOTE]
> W programie `Create` można tworzyć tylko Microsoft Jet (. MDB). Nie można tworzyć baz danych ISAM ani baz danych ODBC.

## <a name="cdaodatabasecreaterelation"></a><a name="createrelation"></a> CDaoDatabase:: isrelation
...
*lAttributes*<br/>
Długa wartość zawierająca informacje o typie relacji. Możesz użyć tej wartości, aby wymusić integralność referencyjną, między innymi. Możesz użyć operatora logicznego OR ( **&#124;**), aby połączyć dowolne z następujących wartości (o ile ma to zastosowanie kombinacja):
...
Wywołaj tę funkcję elementu członkowskiego, aby usunąć określony obiekt querydef — zapisane zapytanie — z `CDaoDatabase` kolekcji querydef obiektu.
...
Aby uzyskać informacje na temat tworzenia obiektów querydef, zobacz Klasa [CDaoQueryDef](../../mfc/reference/cdaoquerydef-class.md). Obiekt querydef zostaje skojarzony z określonym `CDaoDatabase` obiektem podczas konstruowania `CDaoQueryDef` obiektu, przekazując go do obiektu bazy danych.
...
Wywołaj tę funkcję elementu członkowskiego, aby usunąć określoną tabelę i wszystkie jej dane z `CDaoDatabase` kolekcji TableDefs obiektu.
...
Informacje o tworzeniu obiektów tabledef można znaleźć w temacie Class [CDaoTableDef](../../mfc/reference/cdaotabledef-class.md). Obiekt tabledef jest kojarzony z określonym `CDaoDatabase` obiektem podczas konstruowania `CDaoTableDef` obiektu, przekazując mu wskaźnik do obiektu bazy danych.
...
*nOptions*<br/>
Liczba całkowita, która określa opcje dotyczące integralności zapytania. Za pomocą operatora bitowego lub ( **&#124;**) można połączyć dowolne z następujących stałych (pod warunkiem, że połączenie jest sensowne — na przykład nie można łączyć `dbInconsistent` z `dbConsistent` ):
...
> [!NOTE]
> Jeśli oba elementy `dbInconsistent` i `dbConsistent` są dołączone lub nie są uwzględniane, wynik jest wartością domyślną. Aby uzyskać wyjaśnienie tych stałych, zobacz temat "Metoda Execute" w pomocy DAO.
...
`Execute` nie zwraca zestawu rekordów. Użycie `Execute` w zapytaniu, które wybiera rekordy powoduje zgłoszenie wyjątku typu przez MFC `CDaoException` . (Nie istnieje `ExecuteSQL` funkcja członkowska analogiczna do `CDatabase::ExecuteSQL` .)

## <a name="cdaodatabasegetconnect"></a><a name="getconnect"></a> CDaoDatabase:: GetConnect

Wywołaj tę funkcję elementu członkowskiego, aby pobrać parametry połączenia używane do połączenia `CDaoDatabase` obiektu z bazą danych ODBC lub ISAM.
...
Jeśli sieć obsługuje ujednolicone konwencje nazewnictwa (UNC), można również określić ścieżkę sieciową — na przykład " \\ \\ \\ \MYSERVER \\ \MYSHARE \\ \mydir \\ \MYDB. MDB ". (W literałach ciągu są wymagane podwójne ukośniki odwrotne, ponieważ " \\ " jest znakiem ucieczki języka C++).
...
Wywołaj tę funkcję elementu członkowskiego, aby określić, czy `CDaoDatabase` obiekt jest aktualnie otwarty w bazie danych.
...
Należy wywołać tę funkcję elementu członkowskiego, aby zainicjować nowo skonstruowany `CDaoDatabase` obiekt, który reprezentuje istniejącą bazę danych.
...
*lpszName*<br/>
Wyrażenie ciągu, który jest nazwą istniejącego aparatu Microsoft Jet (. MDB) plik bazy danych. Jeśli nazwa pliku ma rozszerzenie, jest to wymagane. Jeśli sieć obsługuje ujednolicone konwencje nazewnictwa (UNC), można również określić ścieżkę sieciową, taką jak " \\ \\ \\ \MYSERVER \\ \MYSHARE \\ \mydir \\ \MYDB. MDB ". (W literałach ciągu są wymagane podwójne ukośniki odwrotne, ponieważ " \\ " jest znakiem ucieczki języka C++).
...
W przypadku próby nawiązania połączenia w celu przekroczenia limitu czasu, jeśli na przykład Host systemu DBMS jest niedostępny. Jeśli próba połączenia nie powiedzie się, program `Open` zgłosi wyjątek typu [CDaoException](../../mfc/reference/cdaoexception-class.md).
...
Jeśli chcesz, możesz otworzyć własne okno dialogowe przed wywołaniem, `Open` Aby uzyskać informacje od użytkownika, takie jak hasło, a następnie dodać te informacje do parametrów połączenia, które są przekazywane `Open` . Można też zapisać parametry połączenia, które zostały przekazane (prawdopodobnie w rejestrze systemu Windows), aby można było użyć go ponownie podczas następnego wywołania aplikacji `Open` na `CDaoDatabase` obiekcie.
...
Operacja może przekroczyć limit czasu, ponieważ występują problemy z dostępem do sieci, nadmierny czas przetwarzania zapytania i tak dalej. Wywołaj `SetQueryTimeout` przed otwarciem zestawu rekordów lub przed wywołaniem funkcji [AddNew](../../mfc/reference/cdaorecordset-class.md#addnew), [Update](../../mfc/reference/cdaorecordset-class.md#update)lub [delete](../../mfc/reference/cdaorecordset-class.md#delete) elementu członkowskiego zestawu rekordów, jeśli chcesz zmienić wartość limitu czasu zapytania. Ustawienie ma wpływ na wszystkie kolejne [otwarte](../../mfc/reference/cdaorecordset-class.md#open), `AddNew` , `Update` i `Delete` wywołania do dowolnego zestawu rekordów skojarzonego z tym `CDaoDatabase` obiektem. Zmiana wartości limitu czasu zapytania dla zestawu rekordów po otwarciu nie powoduje zmiany wartości zestawu rekordów. Na przykład kolejne operacje [przenoszenia](../../mfc/reference/cdaorecordset-class.md#move) nie używają nowej wartości.
