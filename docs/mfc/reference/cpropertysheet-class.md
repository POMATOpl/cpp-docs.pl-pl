---
description: 'Dowiedz się więcej na temat: Klasa CPropertySheet'
...
Wymiana danych między `CPropertySheet` obiektem a obiektem zewnętrznym jest podobna do wymiany danych z `CDialog` obiektem. Istotną różnicą jest to, że ustawienia arkusza właściwości są zwykle zmiennymi elementów członkowskich `CPropertyPage` obiektów, a nie `CPropertySheet` samego obiektu.
...
Aby uzyskać więcej informacji o sposobach korzystania z `CPropertySheet` obiektów, zobacz [arkusze właściwości artykułu i strony właściwości](../../mfc/property-sheets-and-property-pages-in-mfc.md).
...
`AddPage` Dodaje obiekt [CPropertyPage](../../mfc/reference/cpropertypage-class.md#cpropertypage) do `CPropertySheet` listy stron obiektu, ale w rzeczywistości nie tworzy okna dla strony. Struktura opóźnia tworzenie okna dla strony do momentu, gdy użytkownik wybierze Tę stronę.

Po dodaniu strony właściwości przy użyciu `AddPage` elementu, `CPropertySheet` jest elementem nadrzędnym `CPropertyPage` . Aby uzyskać dostęp do arkusza właściwości na stronie właściwości, wywołaj [CWnd:: GetParent](../../mfc/reference/cwnd-class.md#getparent).

Nie trzeba czekać do momentu utworzenia okna arkusza właściwości do wywołania `AddPage` . Zwykle `AddPage` przed wywołaniem metody [DoModal](#domodal) lub [Create](#create)zostanie wywołana.
...
## <a name="cpropertysheetconstruct"></a><a name="construct"></a> CPropertySheet:: konstrukcja
...
Wywołaj tę funkcję elementu członkowskiego, jeśli jeden z konstruktorów klas nie został jeszcze wywołany. Na przykład, wywołaj w `Construct` przypadku deklarowania lub przydzielenia tablic `CPropertySheet` obiektów. W przypadku tablic należy wywołać `Construct` dla każdego elementu członkowskiego tablicy.
...
Można automatycznie wyświetlać obrazy wodne i/lub nagłówki, jeśli są używane trzy lub czwarte prototypy `Construct` , wymienione powyżej, i przekazywane są prawidłowe wartości parametrów *hbmWatermark*, *HpalWatermark* i/lub *hbmHeader* .
...
Można automatycznie wyświetlać obrazy wodne i/lub nagłówki, jeśli są używane trzy lub czwarte prototypy `CPropertySheet` , powyżej i przekazywane są prawidłowe wartości parametrów *hbmWatermark*, *HpalWatermark* i/lub *hbmHeader* .
...
`Create`Funkcja członkowska wraca bezpośrednio po utworzeniu arkusza właściwości. Aby zniszczyć arkusz właściwości, wywołaj [CWnd::D estroywindow](../../mfc/reference/cwnd-class.md#destroywindow).

Niemodalne arkusze właściwości wyświetlane z wywołaniem nie `Create` mają przycisków OK, Anuluj, Zastosuj teraz i pomoc jako modalne arkusze właściwości. Żądane przyciski muszą zostać utworzone przez użytkownika.
...
IDOK lub IDCANCEL, jeśli funkcja zakończyła się pomyślnie; w przeciwnym razie 0 lub-1. Jeśli arkusz właściwości został ustanowiony jako Kreator (zobacz [SetWizardMode](#setwizardmode)), zwraca wartość `DoModal` ID_WIZFINISH lub IDCANCEL.
...
> [!NOTE]
> Ten wyjątek musi być obsługiwany przez system operacyjny w przypadku kompilowania z modelem obsługi wyjątków asynchronicznych. Aby uzyskać więcej informacji na temat modeli obsługi wyjątków, zobacz [/EH (model obsługi wyjątków)](../../build/reference/eh-exception-handling-model.md). W tym przypadku nie należy zawijać wywołań do programu `CPropertySheet::DoModal` przy użyciu bloku try-catch języka C++, w którym catch obsługuje wszystkie wyjątki, na przykład `catch (...)` . Ten blok mógłby obsłużyć wyjątek przeznaczony dla systemu operacyjnego i spowodować nieprzewidywalne zachowanie. Można jednak bezpiecznie używać obsługi wyjątków C++ z określonymi typami wyjątków lub strukturalną obsługą wyjątków, w przypadku których wyjątek naruszenia dostępu jest przełączany do systemu operacyjnego.
...
## <a name="cpropertysheetenddialog"></a><a name="enddialog"></a> CPropertySheet:: zdarzenie EndDialog
...
Użyj tej struktury, aby zainicjować wygląd arkusza właściwości po jego zbudowaniu, ale przed wyświetleniem go przy użyciu funkcji składowej [DoModal](#domodal) . Na przykład Ustaw element członkowski *dwSize* `m_psh` do rozmiaru, który ma mieć arkusz właściwości.
...
## <a name="cpropertysheetremovepage"></a><a name="removepage"></a> CPropertySheet:: Wywołaj RemovePage
...
Sam obiekt [CPropertyPage](../../mfc/reference/cpropertypage-class.md) nie jest niszczony, dopóki nie `CPropertySheet` zostanie zamknięty właściciel okna.
