---
description: 'Dowiedz się więcej na temat: Klasa CToolBar'
...
|[CToolBar:: CreateEx](#createex)|Tworzy `CToolBar` obiekt z dodatkowymi stylami dla osadzonego `CToolBarCtrl` obiektu.|
...
Aby ustawić pojedynczy przycisk lub styl separatora bez wskazywania tablicy, wywołaj metodę [getbutton](#getbuttonstyle) , aby pobrać styl, a następnie Wywołaj metodę [SetButton](#setbuttonstyle) zamiast `SetButtons` . `SetButtonStyle` jest najbardziej przydatna, gdy chcesz zmienić styl przycisku w czasie wykonywania.
...
Aby utworzyć przycisk pola wyboru, przypisz go TBBS_CHECKBOX lub Użyj `CCmdUI` `SetCheck` funkcji składowej obiektu w obsłudze ON_UPDATE_COMMAND_UI. Wywołanie `SetCheck` powoduje zamianę przycisku na przycisk pola wyboru. Przekaż `SetCheck` argument 0 dla opcji niezaznaczone, 1 dla opcji zaznaczone lub 2 dla nieokreślonego.

Aby utworzyć przycisk radiowy, wywołaj funkcję elementu członkowskiego [SetRadio](../../mfc/reference/ccmdui-class.md#setradio) obiektu [CCmdUI](../../mfc/reference/ccmdui-class.md) z procedury obsługi ON_UPDATE_COMMAND_UI. Przekazanie `SetRadio` argumentu 0 dla opcji niezaznaczone lub niezerowego dla zaznaczenia. Aby zapewnić wzajemnie wykluczające się zachowanie grupy radiowej, musisz mieć ON_UPDATE_COMMAND_UI obsługi dla wszystkich przycisków w grupie.

Aby uzyskać więcej informacji na temat korzystania z programu `CToolBar` , zobacz artykuł [implementacja paska narzędzi MFC](../../mfc/mfc-toolbar-implementation.md) i [Uwaga techniczna 31: Paski kontroli](../../mfc/tn031-control-bars.md).
...
Ta funkcja członkowska tworzy pasek narzędzi systemu Windows (okno podrzędne) i kojarzy go z `CToolBar` obiektem.
...
Wywołaj tę funkcję, aby utworzyć pasek narzędzi systemu Windows (okno podrzędne) i skojarzyć go z `CToolBar` obiektem.
...
Użyj `CreateEx` zamiast [tworzenia](#create), gdy pewne style muszą być obecne podczas tworzenia osadzonej kontrolki paska narzędzi. Na przykład ustaw *dwCtrlStyle* na TBSTYLE_FLAT &#124; TBSTYLE_TRANSPARENT, aby utworzyć pasek narzędzi podobny do pasków narzędzi programu Internet Explorer 4.
...
## <a name="ctoolbargetbuttonstyle"></a><a name="getbuttonstyle"></a> CToolBar:: getbutton
...
Użyj, aby skorzystać z `GetToolBarCtrl` funkcji typowej kontrolki paska narzędzi systemu Windows i skorzystać z [CToolBarCtrl](../../mfc/reference/ctoolbarctrl-class.md) pomocy technicznej w celu dostosowania paska narzędzi.
...
## <a name="ctoolbarsetbitmap"></a><a name="setbitmap"></a> CToolBar:: setmap
...
> [!NOTE]
> Można również ustawić Stany przycisku przy użyciu parametru *nStyle* ; Jednak ze względu na to, że Stany przycisków są kontrolowane przez procedurę obsługi [ON_UPDATE_COMMAND_UI](message-map-macros-mfc.md#on_update_command_ui) , każdy ustawiony za pomocą `SetButtonInfo` zostanie utracony w następnym bezczynnym przetwarzaniu. Aby uzyskać więcej informacji, zobacz [temat jak aktualizować User-Interface obiektów](../../mfc/how-to-update-user-interface-objects.md) i [TN031: Paski kontroli](../../mfc/tn031-control-bars.md) .
...
## <a name="ctoolbarsetbuttonstyle"></a><a name="setbuttonstyle"></a> CToolBar:: SetButton
...
Przed wywołaniem `SetButtonStyle` należy wywołać funkcję elementu członkowskiego [getbutton](#getbuttonstyle) , aby pobrać styl przycisku lub separatora.

> [!NOTE]
> Można również ustawić Stany przycisku przy użyciu parametru *nStyle* ; Jednak ze względu na to, że Stany przycisków są kontrolowane przez procedurę obsługi [ON_UPDATE_COMMAND_UI](message-map-macros-mfc.md#on_update_command_ui) , każdy ustawiony za pomocą `SetButtonStyle` zostanie utracony w następnym bezczynnym przetwarzaniu. Aby uzyskać więcej informacji, zobacz [temat jak aktualizować User-Interface obiektów](../../mfc/how-to-update-user-interface-objects.md) i [TN031: Paski kontroli](../../mfc/tn031-control-bars.md) .
