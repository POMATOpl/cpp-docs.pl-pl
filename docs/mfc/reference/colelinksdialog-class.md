---
description: 'Dowiedz się więcej na temat: Klasa COleLinksDialog'
...
Utwórz obiekt klasy, `COleLinksDialog` gdy chcesz wywołać to okno dialogowe. Po `COleLinksDialog` skonstruowaniu obiektu można użyć struktury [m_el](#m_el) , aby zainicjować wartości lub Stany kontrolek w oknie dialogowym. `m_el`Struktura jest typu OLEUIEDITLINKS. Aby uzyskać więcej informacji o używaniu tej klasy okna dialogowego, zobacz funkcja członkowska [DoModal](#domodal) .
