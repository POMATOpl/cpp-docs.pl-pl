---
description: 'Dowiedz się więcej na temat: aktualizowanie tekstu okienka Status-Bar'
...
Zalecanym sposobem wyświetlania tekstu w okienku jest wywołanie `SetText` funkcji składowej klasy `CCmdUI` w funkcji procedury obsługi aktualizacji dla tego okienka. Na przykład możesz chcieć skonfigurować zmienną całkowitą *m_nPage* , która zawiera bieżący numer strony i użyć `SetText` do ustawienia tekstu w okienku na wersję ciągu tego numeru.

> [!NOTE]
> `SetText`Zalecane jest podejście. To zadanie można wykonać na nieco niższym poziomie, wywołując `CStatusBar` funkcję członkowską `SetPaneText` . Nawet dlatego potrzebna jest procedura obsługi aktualizacji. Bez takiego programu obsługi dla okienka MFC automatycznie wyłącza okienko, wymazywanie jego zawartości.
