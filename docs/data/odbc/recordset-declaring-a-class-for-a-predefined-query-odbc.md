---
description: 'Dowiedz się więcej o: zestaw rekordów: deklarowanie klasy dla wstępnie zdefiniowanego zapytania (ODBC)'
...
> [!TIP]
> Zestaw rekordów nie jest potrzebny do wywołania wstępnie zdefiniowanego zapytania, które nie zwraca rekordów. Przygotuj instrukcję SQL zgodnie z poniższym opisem, ale wykonaj ją, wywołując `CDatabase` funkcję członkowską [ExecuteSql by](../../mfc/reference/cdatabase-class.md#executesql).
...
1. Ręcznie Dodaj wywołania funkcji [RFX](../../data/odbc/record-field-exchange-rfx.md) w funkcji członkowskiej [DoFieldExchange](../../mfc/reference/crecordset-class.md#dofieldexchange) klasy, jeden odpowiadający typowi danych każdego dodawanego elementu członkowskiego danych pola.
    ...
    > [!NOTE]
    >  Musisz znać typy danych i kolejność kolumn zwracanych w zestawie wyników. Kolejność wywołań funkcji RFX w elemencie `DoFieldExchange` musi być zgodna z kolejnością kolumn zestawu wyników.
...
1. Ręcznie Dodaj inicjalizacje dla nowych elementów członkowskich danych pola w konstruktorze klas zestawu rekordów.
   ...
   Niektórych typów danych nie należy inicjować w tym miejscu, na przykład w przypadku `CLongBinary` tablic bajtowych.
...
1. Należy zwiększyć wartość `m_nParams` dla każdego dodanego parametru, tak jak w `m_nFields` przypadku dodanych pól w kroku 4 tej procedury. Aby uzyskać więcej informacji, zobacz [zestaw rekordów: parametryzacja a zestaw rekordów (ODBC)](../../data/odbc/recordset-parameterizing-a-recordset-odbc.md).
...
1. W kodzie, który otwiera zestaw rekordów, ustaw wartości elementów członkowskich danych parametrów zestawu rekordów, a następnie Wywołaj `Open` funkcję członkowską, przekazując ciąg SQL dla parametru *lpszSQL* . Zamiast tego Zastąp ciąg zwracany przez `GetDefaultSQL` funkcję członkowską w klasie.

W poniższych przykładach pokazano procedurę wywoływania wstępnie zdefiniowanego zapytania o nazwie `Delinquent_Accts` , które przyjmuje jeden parametr dla numeru okręgu sprzedaży. To zapytanie zwraca trzy kolumny: `Acct_No` , `L_Name` , `Phone` . Wszystkie kolumny pochodzą z tabeli Customers.
...
Ta deklaracja klasy jest zapisywany przez kreatora, z wyjątkiem tego, że `m_lDistParam` element członkowski został dodany ręcznie. Inne elementy członkowskie nie są wyświetlane w tym miejscu.

W następnym przykładzie pokazano inicjalizacje elementów członkowskich danych w `CDelinquents` konstruktorze.
...
Zanotuj inicjalizacje dla [m_nFields](../../mfc/reference/crecordset-class.md#m_nfields) i [m_nParams](../../mfc/reference/crecordset-class.md#m_nparams). Kreator inicjuje się `m_nFields` ; zainicjowano `m_nParams` .
...
Oprócz wykonywania wywołań RFX dla trzech zwracanych kolumn ten kod zarządza wiązaniem parametru, który jest przekazywany w czasie wykonywania. Parametr jest kluczem do `Dist_No` kolumny (numer okręgu).
...
[Zestaw rekordów: badanie zestawu rekordów (ODBC)](../../data/odbc/recordset-requerying-a-recordset-odbc.md)<br/>
