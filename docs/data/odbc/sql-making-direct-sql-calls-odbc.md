---
description: 'Dowiedz się więcej na temat: SQL: wykonywanie bezpośrednich wywołań SQL (ODBC)'
...
Możesz bezpośrednio wykonać wywołanie SQL przy użyciu obiektu [klasy CDatabase](../../mfc/reference/cdatabase-class.md) . Skonfiguruj ciąg instrukcji SQL (zazwyczaj w a `CString` ) i przekaż go do funkcji składowej [CDatabase:: ExecuteSql by](../../mfc/reference/cdatabase-class.md#executesql) `CDatabase` obiektu. Jeśli używasz wywołań funkcji ODBC do wysyłania instrukcji SQL, która zwykle zwraca rekordy, rekordy są ignorowane.
