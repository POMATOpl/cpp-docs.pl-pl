---
description: 'Dowiedz się więcej na temat: MFC: używanie klas baz danych bez dokumentów i widoków'
...
|------------|----------|--------------|
|**Brak**|Pochodny od `CView` .|Nie zapewnia obsługi bazy danych. Jest to domyślne ustawienie opcji.<br /><br /> Jeśli wybierzesz opcję **Obsługa architektury dokumentu/widoku** na stronie [Typ aplikacji, Kreator aplikacji MFC](../mfc/reference/application-type-mfc-application-wizard.md) , uzyskasz pełną obsługę dokumentów, w tym serializacji i **nowe**, **otwarte**, **Zapisz** i **Zapisz jako** polecenia w menu **plik** . Zobacz [aplikacje bez dokumentu](#_core_applications_with_no_document).|
...
Kreator aplikacji MFC ma dwie opcje, które obsługują aplikacje dostępu do danych oparte na formularzach. Każda opcja tworzy `CRecordView` klasę widoku pochodnego i dokumentu. Różnią się one w porównaniu z dokumentem.
...
- Jeśli aplikacja wyświetla wiele widoków (takich jak wiele widoków rekordów), dokument obsługuje koordynowanie widoków.
   ...
   Jeśli wiele widoków wyświetla te same dane, można użyć `CDocument::UpdateAllViews` funkcji składowej, aby koordynować aktualizacje do wszystkich widoków, gdy dowolny widok zmieni dane.
...
Czasami warto napisać aplikację, która nie korzysta z dokumentów lub widoków. Bez dokumentów dane są przechowywane (takie jak `CRecordset` obiekt) w klasie okna ramki lub w klasie aplikacji. Wszelkie dodatkowe wymagania zależą od tego, czy aplikacja prezentuje interfejs użytkownika.
...
Jeśli dysponujesz interfejsem użytkownika (innym niż, na przykład w interfejsie wiersza polecenia konsoli), aplikacja rysuje się bezpośrednio w obszarze klienta okna ramki, a nie w widoku. Takie aplikacje nie używają `CRecordView` , `CFormView` lub `CDialog` dla głównego interfejsu użytkownika, ale zazwyczaj są używane `CDialog` do zwykłych okien dialogowych.
...
Ponieważ Kreator aplikacji nie obsługuje tworzenia aplikacji bez dokumentów, należy napisać własną `CWinApp` klasę pochodną i, jeśli to konieczne, również utworzyć `CFrameWnd` `CMDIFrameWnd` klasę or. Przesłoń `CWinApp::InitInstance` i Zadeklaruj obiekt aplikacji jako:
...
Ponieważ żaden dokument nie jest właścicielem `CRecordset` obiektu, prawdopodobnie chcesz go zapisać jako osadzony element członkowski danych w `CWinApp` klasie aplikacji pochodnej. Alternatywy obejmują:

- Nie zachowuje trwałego `CRecordset` obiektu. Można przekazać wartość NULL do konstruktorów klas zestawu rekordów. W takim przypadku struktura tworzy `CDatabase`  obiekt tymczasowy przy użyciu informacji w `GetDefaultConnect` funkcji składowej zestawu rekordów. Jest to najbardziej prawdopodobną metodę alternatywną.

- Tworzenie `CRecordset` obiektu jako zmiennej globalnej. Ta zmienna powinna być wskaźnikiem do obiektu zestawu rekordów, który jest tworzony dynamicznie w `CWinApp::InitInstance` przesłonięciu. Pozwala to uniknąć próby konstruowania obiektu przed zainicjowaniem struktury.
