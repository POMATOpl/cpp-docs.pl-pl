---
description: 'Dowiedz się więcej o: &lt; &gt; funkcjach systemu plików'
...
Wszystkie *funkcje, które* prawdopodobnie kopiują lub łączą jeden lub więcej plików *od* *do pod kontrolą opcji,* który jest traktowany jak `copy_options::none` dla przeciążenia bez żadnego  parametru. takie *działania powinny zawierać* co najwyżej jeden z:
...
Funkcje najpierw określają wartości file_status `f`  dla `t` : 
...
Wszystkie funkcje, które prawdopodobnie kopiują plik *od* do *do* pod kontrolą opcji *, które* są wykonywane jak `copy_options::none` dla przeciążenia bez dołączania  parametru. takie *działania powinny zawierać* co najwyżej jedną z `skip_existing` , `overwrite_existing` lub `update_existing` .
