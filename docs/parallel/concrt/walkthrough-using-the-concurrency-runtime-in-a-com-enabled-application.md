---
description: 'Dowiedz się więcej na temat: Przewodnik: używanie środowisko uruchomieniowe współbieżności w aplikacji COM-Enabled'
...
W przypadku używania modelu COM ze składnikiem w bibliotece równoległych wzorców (PPL), na przykład grupy zadań lub algorytmu równoległego, należy wywołać `CoInitializeEx` przed użyciem biblioteki com podczas każdego zadania lub iteracji i wywołać `CoUninitialize` przed zakończeniem każdego zadania lub iteracji. Poniższy przykład pokazuje, jak zarządzać okresem istnienia biblioteki COM przy użyciu obiektu [concurrency:: structured_task_group](../../parallel/concrt/reference/structured-task-group-class.md) .

[!code-cpp[concrt-parallel-scripts#1](../../parallel/concrt/codesnippet/cpp/walkthrough-using-the-concurrency-runtime-in-a-com-enabled-application_1.cpp)]

Należy upewnić się, że biblioteka COM została prawidłowo zwolniona, gdy zadanie lub algorytm równoległy został anulowany, lub gdy treść zadania zgłasza wyjątek. Aby zagwarantować, że zadanie wywołuje się `CoUninitialize` przed opuszczeniem, użyj `try-finally` wzorca lub RAII.  Poniższy przykład używa bloku, `try-finally` Aby zwolnić bibliotekę com, gdy zadanie zostanie ukończone lub zostało anulowane, lub gdy zostanie zgłoszony wyjątek.
...
Można użyć klasy, `CCoInitializer` Aby automatycznie zwolnić bibliotekę com po zakończeniu zadania w następujący sposób.
...
W tej sekcji przedstawiono kompletną aplikację obsługującą COM, która używa `IScriptControl` interfejsu do wykonywania skryptu, który oblicza n-<sup>ty</sup> numer Fibonacci. Ten przykład najpierw wywołuje skrypt z wątku głównego, a następnie używa PPL i agentów do wywołania skryptu współbieżnie.

Rozważmy następującą funkcję pomocnika, `RunScriptProcedure` która wywołuje procedurę w `IScriptControl` obiekcie.

[!code-cpp[concrt-parallel-scripts#7](../../parallel/concrt/codesnippet/cpp/walkthrough-using-the-concurrency-runtime-in-a-com-enabled-application_7.cpp)]

`wmain`Funkcja tworzy `IScriptControl` obiekt, dodaje do niego kod skryptu, który oblicza n-<sup>ty</sup> numer Fibonacci, a następnie wywołuje `RunScriptProcedure` funkcję w celu uruchomienia tego skryptu.
...
Aby użyć `ParallelFibonacci` funkcji z przykładu, Dodaj następujący kod przed `wmain` zwróceniem funkcji.
...
Aby użyć `AgentFibonacci` funkcji z przykładu, Dodaj następujący kod przed `wmain` zwróceniem funkcji.
