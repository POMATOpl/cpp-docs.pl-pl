---
description: 'Dowiedz się więcej na temat: Klasa CRegKey'
title: Klasa CRegKey
ms.date: 11/04/2016
f1_keywords:
- CRegKey
...
|Nazwa|Opis|
|----------|-----------------|
|[CRegKey:: Attach](#attach)|Wywołaj tę metodę, aby dołączyć HKEY do `CRegKey` obiektu przez ustawienie dojścia do elementu członkowskiego [m_hKey](#m_hkey) `hKey` .|
...
|[CRegKey::D eleteSubKey](#deletesubkey)|Wywołaj tę metodę, aby usunąć określony klucz z rejestru.|
|[CRegKey::D eleteValue](#deletevalue)|Wywołaj tę metodę, aby usunąć pole wartości z [m_hKey](#m_hkey).|
|[CRegKey::D etach](#detach)|Wywołaj tę metodę, aby odłączyć dojście składowej [m_hKey](#m_hkey) od `CRegKey` obiektu i ustawić `m_hKey` na wartość null.|
...
`CRegKey` zapewnia interfejs programowania dla rejestru systemowego dla danego komputera. Na przykład, aby otworzyć określony klucz rejestru, wywołaj polecenie `CRegKey::Open` . Aby pobrać lub zmodyfikować wartość danych, wywołaj `CRegKey::QueryValue` lub `CRegKey::SetValue` , odpowiednio. Aby zamknąć klucz, wywołaj polecenie `CRegKey::Close` .
...
Wywołaj tę metodę, aby dołączyć HKEY do `CRegKey` obiektu przez ustawienie uchwytu składowej [M_hKey](#m_hkey) na *HKEY*.
...
`Attach` zostanie potwierdzone, jeśli `m_hKey` ma wartość różną od null.
...
*lpdwDisposition*<br/>
określoną Jeśli wartość nie jest równa NULL, pobiera REG_CREATED_NEW_KEY (Jeśli klucz nie istnieje i został utworzony) lub REG_OPENED_EXISTING_KEY (Jeśli klucz istniał i został otwarty).

### <a name="return-value"></a>Wartość zwracana

Jeśli powiedzie się, zwraca ERROR_SUCCESS i otwiera klucz. Jeśli metoda zakończy się niepowodzeniem, wartość zwracana jest niezerowym kodem błędu zdefiniowanym w WINERROR. H.
...
Jeśli powiedzie się, zwraca ERROR_SUCCESS. Jeśli metoda zakończy się niepowodzeniem, wartość zwracana jest niezerowym kodem błędu zdefiniowanym w WINERROR. H.
...
Wywołaj tę metodę, aby odłączyć dojście składowej [m_hKey](#m_hkey) od `CRegKey` obiektu i ustawić `m_hKey` na wartość null.
...
Jeśli metoda się powiedzie, wartość zwracana jest ERROR_SUCCESS. Jeśli metoda zakończy się niepowodzeniem, wartość zwracana jest niezerowym kodem błędu zdefiniowanym w WINERROR. H.

### <a name="remarks"></a>Uwagi

Aby wyliczyć podklucze, wywołaj `CRegKey::EnumKey` z indeksem równym zero. Zwiększ wartość indeksu i powtórz operację do momentu, gdy metoda zwróci wartość ERROR_NO_MORE_ITEMS. Aby uzyskać więcej informacji, zobacz [RegEnumKeyEx](/windows/win32/api/winreg/nf-winreg-regenumkeyexw) w Windows SDK.
...
Jeśli powiedzie się, zwraca ERROR_SUCCESS; w przeciwnym razie wartość błędu różna od zera zdefiniowana w WINERROR. H.
...
Jeśli metoda zakończy się pomyślnie, ERROR_SUCCESS jest zwracana. Jeśli metoda nie może odczytać wartości, zwraca kod błędu różny od zera zdefiniowany w WINERROR. H. Jeśli przywoływane dane nie są typu REG_BINARY, zwracany jest ERROR_INVALID_DATA.
...
Jeśli metoda zakończy się pomyślnie, ERROR_SUCCESS jest zwracana. Jeśli metoda nie może odczytać wartości, zwraca kod błędu różny od zera zdefiniowany w WINERROR. H. Jeśli przywoływane dane nie są typu REG_DWORD, zwracany jest ERROR_INVALID_DATA.
...
Jeśli metoda zakończy się pomyślnie, ERROR_SUCCESS jest zwracana. Jeśli metoda nie może odczytać wartości, zwraca kod błędu różny od zera zdefiniowany w WINERROR. H. Jeśli dane, których dotyczy odwołanie, nie jest prawidłowym identyfikatorem GUID, zwracany jest ERROR_INVALID_DATA.
...
Jeśli metoda zakończy się pomyślnie, ERROR_SUCCESS jest zwracana. Jeśli metoda nie może odczytać wartości, zwraca kod błędu różny od zera zdefiniowany w WINERROR. H. Jeśli przywoływane dane nie są typu REG_MULTI_SZ, zwracany jest ERROR_INVALID_DATA.
...
Jeśli metoda zakończy się pomyślnie, ERROR_SUCCESS jest zwracana. Jeśli metoda nie może odczytać wartości, zwraca kod błędu różny od zera zdefiniowany w WINERROR. H. Jeśli przywoływane dane nie są typu REG_QWORD, zwracany jest ERROR_INVALID_DATA.
...
Jeśli metoda zakończy się pomyślnie, ERROR_SUCCESS jest zwracana. Jeśli metoda nie może odczytać wartości, zwraca kod błędu różny od zera zdefiniowany w WINERROR. H. Jeśli przywoływane dane nie są typu REG_SZ, zwracany jest ERROR_INVALID_DATA. Jeśli metoda zwraca ERROR_MORE_DATA, *pnChars* ma wartość zero, a nie wymaganego rozmiaru buforu w bajtach.
...
Jeśli powiedzie się, zwraca ERROR_SUCCESS; w przeciwnym razie kod błędu różny od zera zdefiniowany w WINERROR. H.

### <a name="remarks"></a>Uwagi

Dwie wersje pierwotne `QueryValue` nie są już obsługiwane i są oznaczone jako ATL_DEPRECATED. Kompilator wyświetli ostrzeżenie, jeśli są używane te formularze.
...
|SACL_SECURITY_INFORMATION|Ustawia listę kontroli dostępu do systemu (SACL) klucza. Klucz musi mieć dostęp ACCESS_SYSTEM_SECURITY. Odpowiednim sposobem uzyskania dostępu jest włączenie [uprawnienia](/windows/win32/secauthz/privileges) SE_SECURITY_NAME w bieżącym tokenie dostępu obiektu wywołującego, otwarcie dojścia do ACCESS_SYSTEM_SECURITY dostępu, a następnie wyłączenie tego uprawnienia.|

*formacie*<br/>
Wskaźnik do struktury [SECURITY_DESCRIPTOR](/windows/win32/api/winnt/ns-winnt-security_descriptor) , która określa atrybuty zabezpieczeń, które mają zostać ustawione dla określonego klucza.
...
*bMulti*<br/>
W przypadku wartości false wskazuje, że ciąg jest typu REG_SZ. W przypadku wartości true wskazuje, że ciąg jest wielociągiem typu REG_MULTI_SZ.
