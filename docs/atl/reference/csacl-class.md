---
description: 'Dowiedz się więcej na temat: Klasa CSacl'
title: Klasa CSacl
ms.date: 11/04/2016
f1_keywords:
- CSacl
...
Zwraca wartość TRUE, jeśli do obiektu zostanie dodany wpis ACE `CSacl` , wartość false w przypadku niepowodzenia.
...
Zwraca odwołanie do zaktualizowanego `CSacl` obiektu. Upewnij się, że `ACL` parametr jest rzeczywiście systemową listą kontroli dostępu (SACL), a nie poufną listą kontroli dostępu (DACL). W obszarze kompilacje debugowania zostanie wykonane potwierdzenie, a w kompilacjach wydania `ACL` parametr zostanie zignorowany.
