---
description: 'Dowiedz się więcej na temat: Klasa CBindStatusCallback'
...
|[CBindStatusCallback::StartAsyncDownload](#startasyncdownload)|Inicjuje dostępne bajty i Bajty odczytane do zera, tworzy obiekt strumienia typu push na podstawie adresu URL i wywołuje `OnDataAvailable` każde dane czasu.|
...
Klasa `CBindStatusCallback` implementuje interfejs `IBindStatusCallback`. `IBindStatusCallback` musi być zaimplementowany przez aplikację, aby można było otrzymywać powiadomienia o asynchronicznym transferze danych. Moniker asynchroniczny dostarczony przez system używa `IBindStatusCallback` metod do wysyłania i odbierania informacji o asynchronicznym przekazywaniu danych do i z obiektu.

Zazwyczaj `CBindStatusCallback` obiekt jest skojarzony z określoną operacją wiązania. Na przykład w próbce [asynchronicznej](../../overview/visual-cpp-samples.md) podczas ustawiania właściwości adresu URL tworzy `CBindStatusCallback` obiekt w wywołaniu `Download` :

[!code-cpp[NVC_ATL_Windowing#86](../../atl/codesnippet/cpp/cbindstatuscallback-class_1.h)]

Moniker asynchroniczny używa funkcji wywołania zwrotnego `OnData` do wywołania aplikacji, gdy ma ona dane. Moniker asynchroniczny jest dostarczany przez system.
...
Konstruktor inicjuje również [m_pT](#m_pt) i [m_pFunc](#m_pfunc) do wartości null.
...
## <a name="cbindstatuscallbackdownload"></a><a name="download"></a> CBindStatusCallback::D obierz

Tworzy `CBindStatusCallback` obiekt i wywołuje, `StartAsyncDownload` Aby rozpocząć pobieranie danych asynchronicznie z podanego adresu URL.
...
Za każdym razem, gdy dane są dostępne, są wysyłane do obiektu za pomocą `OnDataAvailable` . `OnDataAvailable` odczytuje dane i wywołuje funkcję wskazywaną przez *pFunc* (na przykład w celu przechowywania danych lub drukowania na ekranie).
...
BINDF_NOWRITECACHE Operacja powiązania nie powinna przechowywać pobranych danych w pamięci podręcznej dysków.
...
Zainicjowany w `OnStartBinding` i wydano w `OnStopBinding` .
...
Zainicjowany w `OnDataAvailable` ramach `STGMEDIUM` struktury, gdy flaga BCSF jest BCSF_FIRSTDATANOTIFICATION i wydawana, gdy flaga BCSF jest BCSF_LASTDATANOTIFICATION.

## <a name="cbindstatuscallbackondataavailable"></a><a name="ondataavailable"></a> CBindStatusCallback::OnDataAvailable

Asynchroniczne wywołania monikera dostarczone przez system `OnDataAvailable` w celu dostarczenia danych do obiektu, gdy staną się dostępne.
...
*ulProgress*<br/>
Długa liczba całkowita bez znaku. Nieużywany.
