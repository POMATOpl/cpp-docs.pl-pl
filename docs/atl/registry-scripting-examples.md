---
description: 'Dowiedz się więcej o: przykłady skryptów rejestru'
...
Teraz skrypt dodaje podklucz, `HasASubkey` do `MyVeryOwnKey` . Do tego podklucza dodaje `PrettyCool` podklucz (z `DWORD` wartością domyślną 55) i `ANameValue` nazwaną wartość (z wartością ciągu `WithANamedValue` ).
...
- Określa `{44EC053A-400F-11D0-9DCD-00A0C90391D3}` dla elementu `CLSID` . (Ta wartość jest identyfikatorem CLSID rejestratora do użycia z `CoCreateInstance` .)
...
- Usuwa cyklicznie `{44EC053A-400F-11D0-9DCD-00A0C90391D3}` i wszystkie jego podklucze.
...
> [!NOTE]
> W skrypcie rejestratora jest maksymalny rozmiar tokenu 4K. (Token jest dowolnym rozpoznawalnym elementem w składni). W poprzednim przykładzie skryptu,, `HKCR` , `HKEY_CURRENT_USER` `'MyVeryOwnKey'` i `'HowGoesIt'` są tokenami.
