---
description: 'Dowiedz się więcej na temat: _aligned_free_dbg'
...
Funkcja **_aligned_free_dbg** jest wersją debugowania funkcji [_aligned_free](aligned-free.md) . Gdy [_DEBUG](../../c-runtime-library/debug.md) nie jest zdefiniowany, każde wywołanie **_aligned_free_dbg** zostanie zredukowane do wywołania `_aligned_free` . Zarówno, `_aligned_free` jak i **_aligned_free_dbg** zwolnić blok pamięci w stercie podstawowym, ale **_aligned_free_dbg** obsługuje funkcję debugowania: możliwość utrzymywania zwolnionych bloków na połączonej liście sterty w celu symulowania warunków niedostatecznej ilości pamięci.
