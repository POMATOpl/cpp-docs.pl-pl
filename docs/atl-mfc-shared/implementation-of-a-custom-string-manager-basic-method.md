---
description: 'Dowiedz się więcej o: implementacji niestandardowego menedżera ciągów (metoda podstawowa)'
...
Najprostszym sposobem dostosowania schematu alokacji pamięci dla danych ciągu jest użycie klasy dostarczonej ATL, `CAtlStringMgr` ale udostępnienie własnych procedur alokacji pamięci. Konstruktor dla `CAtlStringMgr` przyjmuje jeden parametr: wskaźnik do `IAtlMemMgr` obiektu. `IAtlMemMgr` jest abstrakcyjną klasą bazową, która udostępnia interfejs ogólny do sterty. Za pomocą `IAtlMemMgr` interfejsu, `CAtlStringMgr` przydzielenia, przydzielania i zwalniania pamięci używanej do przechowywania danych ciągu. Można zaimplementować `IAtlMemMgr` interfejs samodzielnie lub użyć jednej z pięciu klas Menedżera pamięci dostarczonych przez ATL. Menedżery pamięci udostępnionej przez ATL po prostu zawijają istniejące obiekty alokacji pamięci:
...
Aby użyć tego prywatnego Menedżera ciągów do zarządzania pamięcią dla `CString` zmiennej, Przekaż wskaźnik do Menedżera jako parametr do `CString` konstruktora zmiennej:
